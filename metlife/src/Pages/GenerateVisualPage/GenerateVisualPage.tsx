...
const initialScenes: Scene[] = [
  {
    id: 1,
    title: "Scene 01",
    text: "It’s a bright morning in the city. A boy is running to catch the bus.",
  },
  {
    id: 2,
    title: "Scene 02",
    text: "He reaches the bus stop and meets a friend who shows him a secret map.",
  },
  {
    id: 3,
    title: "Scene 03",
    text: "They decide to follow it and find a small hidden garden.",
  },
];
...
const GenerateVisualsPage: React.FC = () => {
  ...
  const selectedScene = useMemo<Scene>(
  ...
  const handleSelectScene = (id: number) => setSelectedSceneId(id);
  ...
  const handleAddScene = () => {
    ...
    const newScene: Scene = {
      id: nextId,
      title: `Scene ${String(nextId).padStart(2, "0")}`,
  ...
  const handleDeleteScene = (id: number) => {
    ...
    if (selectedSceneId === id && updated.length > 0) {
      setSelectedSceneId(updated[0].id);
    }
  };

  const handleCopyScene = (id: number) => {
  ...
  const handleUpdateSceneText = (text: string) => {
    setScenes((prev) =>
      prev.map((s) =>
        s.id === selectedScene.id ? { ...s, text } : s
      )
    );
  };
  ...
  const handleGenerateVideo = () => {
    ...
    alert(
      `Generate video for ${selectedScene.title} as ${selectedVisualType}`
    );
  };
      ...
      <main className={styles.container}>
        ...
        <aside className={styles.left}>
          <div className={styles.leftHeader}>
            <h3>Your Script</h3>
            <ButtonComp
              label="Regenerate Entire Script"
              sx={{
                backgroundColor: "#938d8d12 ",
                "&:hover": { backgroundColor: "#938d8d12" },
              }}
              action={() => alert("Regenerate whole script")}
            />
          </div>
