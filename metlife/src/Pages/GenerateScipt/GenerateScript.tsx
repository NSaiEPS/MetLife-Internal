...
const GenerateScript = () => {
  ...
  const { promptData } = useSelector((store: RootState) => store.Prompts);

  useEffect(() => {
    dispatch(getPromptsList() as any);
  }, [dispatch]);
  ...
  const handleGenerate = () => {
    if (!language) return showToast.error("Please select Language in Video Filters");
    if (!videoType) return showToast.error("Please select Video Type in Video Filters");
    if (!audience) return showToast.error("Please enter Target Audience in Video Filters");
    if (!duration) return showToast.error("Please select Duration in Video Filters");
    if (!model) return showToast.error("Please select Model in Model Filters");
    if (!datasource) return showToast.error("Please select Data Source in Model Filters");
    if (!title) return showToast.error("Please give title!");
  ...
  const apiCall = async () => {
    ...
    try {
      ...
      if (result?.status === 200 && result?.data?.scenes && result?.data?.status) {
